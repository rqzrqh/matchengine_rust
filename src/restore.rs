...
fn load_order(m: &mut Market, conn: &mut PooledConn, timestamp: i64) {
    ...
    loop {
        ...
        while i < count {
            ...
            warn!("id:{} type:{} side:{} create_time:{} update_time:{} user_id:{} price:{} amount:{} taker_fee_rate:{} maker_fee_rate:{} left:{} deal_stock:{} deal_money:{} deal_fee:{}",
    ...
    match res {
        Ok(v) => {
            if v.len() != 0 {
                ...
                warn!("last state time:{} oper_id:{} order_id:{} deals_id:{} message_id:{} mq_offset:{}", 
